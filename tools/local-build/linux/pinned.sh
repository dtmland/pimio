...
# shellcheck source=tools/build/pins.sh
source "$(dirname -- "${BASH_SOURCE[0]}")/../../build/pins.sh" || return 1
PIMIO_LOCAL_IMAGE="pimio-local-build-linux:ubuntu-$PIMIO_UBUNTU_VERSION-qt-$PIMIO_QT_VERSION"
    ...
    case "$mode" in
        build)
            "$engine" build --platform linux/amd64 \
                ...
                -t "$image" "$repository_root/tools/build"
            ;;
        pull)
            "$engine" pull "$image"
            ;;
